...
import { ArrowLeft, LogOut, Upload, Loader2 } from "lucide-react";
...
const HeroEditor = () => {
  ...
  const [draft, setDraft] = useState<HeroData>(() => JSON.parse(JSON.stringify(hero)));
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <BilingualField
          ...
          onChange={(v) => setDraft((p) => ({ ...p, headline: v }))}
          maxLength={120}
        />
        <BilingualField
          label="Subheadline"
          value={draft.subheadline}
          onChange={(v) => setDraft((p) => ({ ...p, subheadline: v }))}
          maxLength={300}
          multiline
        />
        <BilingualField
          ...
          onChange={(v) => setDraft((p) => ({ ...p, buttonText: v }))}
          maxLength={40}
        />
...
const TeamEditor = () => {
  ...
  const [headingDraft, setHeadingDraft] = useState<BiText>(() => JSON.parse(JSON.stringify(teamSection.heading)));
  const [descriptionDraft, setDescriptionDraft] = useState<BiText>(() => JSON.parse(JSON.stringify(teamSection.barText)));
  ...
  const save = async () => {
    ...
    try {
      ...
      const { error: e1 } = await supabase
        .from("site_settings")
        .upsert({ key: "team_heading", value: JSON.stringify(headingDraft) }, { onConflict: "key" });
      if (e1) throw e1;
      const { error: e2 } = await supabase
        .from("site_settings")
        .upsert({ key: "team_description", value: JSON.stringify(descriptionDraft) }, { onConflict: "key" });
      if (e2) throw e2;
      setTeamSection({ ...teamSection, heading: headingDraft, barText: descriptionDraft });
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          {teamSection.bannerImageUrl ? (
            <img src={teamSection.bannerImageUrl} alt="Team Banner Preview" className="w-full aspect-[3/1] object-cover rounded-lg border" />
          ) : (
            <div className="w-full aspect-[3/1] rounded-lg border bg-muted flex items-center justify-center text-muted-foreground">No banner uploaded yet</div>
          )}
          <Button type="button" variant="outline" onClick={() => bannerFileRef.current?.click()} disabled={uploadingBanner}>
            {uploadingBanner ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Uploading...</> : <><Upload className="h-4 w-4 mr-2" /> Upload Banner Image</>}
          </Button>
          <input ref={bannerFileRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) handleBannerUpload(file); }} />
      ...
      <Card>
        ...
        <CardContent>
          <BilingualField label="Heading" value={headingDraft} onChange={setHeadingDraft} maxLength={80} />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <BilingualField label="Description" value={descriptionDraft} onChange={setDescriptionDraft} maxLength={500} multiline rows={5} />
          <p className="text-xs text-muted-foreground mt-2">This text appears in the bar below the team banner image.</p>
        </CardContent>
      </Card>
